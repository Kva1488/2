// Положение Луны по теории ELP-2000/82 в сокращении Миуса (глава 47).
// Точность ~10″ по долготе и ~4″ по широте — с большим запасом для карты неба.

import { sind, cosd, norm360, DEG, atan2d, acosd } from './math.js';
import { centuries } from './time.js';
import { obliquity, eclToEq } from './coords.js';
import { sunPosition, AU_KM } from './sun.js';

export const MOON_RADIUS_KM = 1737.4;
export const EARTH_RADIUS_KM = 6378.14;

// Таблица 47.A: D, M, M′, F, Σl (1e-6 °), Σr (1e-3 км)
const LR = [
  [0, 0, 1, 0, 6288774, -20905355], [2, 0, -1, 0, 1274027, -3699111],
  [2, 0, 0, 0, 658314, -2955968], [0, 0, 2, 0, 213618, -569925],
  [0, 1, 0, 0, -185116, 48888], [0, 0, 0, 2, -114332, -3149],
  [2, 0, -2, 0, 58793, 246158], [2, -1, -1, 0, 57066, -152138],
  [2, 0, 1, 0, 53322, -170733], [2, -1, 0, 0, 45758, -204586],
  [0, 1, -1, 0, -40923, -129620], [1, 0, 0, 0, -34720, 108743],
  [0, 1, 1, 0, -30383, 104755], [2, 0, 0, -2, 15327, 10321],
  [0, 0, 1, 2, -12528, 0], [0, 0, 1, -2, 10980, 79661],
  [4, 0, -1, 0, 10675, -34782], [0, 0, 3, 0, 10034, -23210],
  [4, 0, -2, 0, 8548, -21636], [2, 1, -1, 0, -7888, 24208],
  [2, 1, 0, 0, -6766, 30824], [1, 0, -1, 0, -5163, -8379],
  [1, 1, 0, 0, 4987, -16675], [2, -1, 1, 0, 4036, -12831],
  [2, 0, 2, 0, 3994, -10445], [4, 0, 0, 0, 3861, -11650],
  [2, 0, -3, 0, 3665, 14403], [0, 1, -2, 0, -2689, -7003],
  [2, 0, -1, 2, -2602, 0], [2, -1, -2, 0, 2390, 10056],
  [1, 0, 1, 0, -2348, 6322], [2, -2, 0, 0, 2236, -9884],
  [0, 1, 2, 0, -2120, 5751], [0, 2, 0, 0, -2069, 0],
  [2, -2, -1, 0, 2048, -4950], [2, 0, 1, -2, -1773, 4130],
  [2, 0, 0, 2, -1595, 0], [4, -1, -1, 0, 1215, -3958],
  [0, 0, 2, 2, -1110, 0], [3, 0, -1, 0, -892, 3258],
  [2, 1, 1, 0, -810, 2616], [4, -1, -2, 0, 759, -1897],
  [0, 2, -1, 0, -713, -2117], [2, 2, -1, 0, -700, 2354],
  [2, 1, -2, 0, 691, 0], [2, -1, 0, -2, 596, 0],
  [4, 0, 1, 0, 549, -1423], [0, 0, 4, 0, 537, -1117],
  [4, -1, 0, 0, 520, -1571], [1, 0, -2, 0, -487, -1739],
  [2, 1, 0, -2, -399, 0], [0, 0, 2, -2, -381, -4421],
  [1, 1, 1, 0, 351, 0], [3, 0, -2, 0, -340, 0],
  [4, 0, -3, 0, 330, 0], [2, -1, 2, 0, 327, 0],
  [0, 2, 1, 0, -323, 1165], [1, 1, -1, 0, 299, 0],
  [2, 0, 3, 0, 294, 0], [2, 0, -1, -2, 0, 8752],
];

// Таблица 47.B: D, M, M′, F, Σb (1e-6 °)
const B = [
  [0, 0, 0, 1, 5128122], [0, 0, 1, 1, 280602], [0, 0, 1, -1, 277693],
  [2, 0, 0, -1, 173237], [2, 0, -1, 1, 55413], [2, 0, -1, -1, 46271],
  [2, 0, 0, 1, 32573], [0, 0, 2, 1, 17198], [2, 0, 1, -1, 9266],
  [0, 0, 2, -1, 8822], [2, -1, 0, -1, 8216], [2, 0, -2, -1, 4324],
  [2, 0, 1, 1, 4200], [2, 1, 0, -1, -3359], [2, -1, -1, 1, 2463],
  [2, -1, 0, 1, 2211], [2, -1, -1, -1, 2065], [0, 1, -1, -1, -1870],
  [4, 0, -1, -1, 1828], [0, 1, 0, 1, -1794], [0, 0, 0, 3, -1749],
  [0, 1, -1, 1, -1565], [1, 0, 0, 1, -1491], [0, 1, 1, 1, -1475],
  [0, 1, 1, -1, -1410], [0, 1, 0, -1, -1344], [1, 0, 0, -1, -1335],
  [0, 0, 3, 1, 1107], [4, 0, 0, -1, 1021], [4, 0, -1, 1, 833],
  [0, 0, 1, -3, 777], [4, 0, -2, 1, 671], [2, 0, 0, -3, 607],
  [2, 0, 2, -1, 596], [2, -1, 1, -1, 491], [2, 0, -2, 1, -451],
  [0, 0, 3, -1, 439], [2, 0, 2, 1, 422], [2, 0, -3, -1, 421],
  [2, 1, -1, 1, -366], [2, 1, 0, 1, -351], [4, 0, 0, 1, 331],
  [2, -1, 1, 1, 315], [2, -2, 0, -1, 302], [0, 0, 1, 3, -283],
  [2, 1, 1, -1, -229], [1, 1, 0, -1, 223], [1, 1, 0, 1, 223],
  [0, 1, -2, -1, -220], [2, 1, -1, -1, -220], [1, 0, 1, 1, -185],
  [2, -1, -2, -1, 181], [0, 1, 2, 1, -177], [4, 0, -2, -1, 176],
  [4, -1, -1, -1, 166], [1, 0, 1, -1, -164], [4, 0, 1, -1, 132],
  [1, 0, -1, -1, -119], [4, -1, 0, -1, 115], [2, -2, 0, 1, 107],
];

/**
 * Геоцентрическое положение Луны.
 * @param {number} jdTT юлианская дата в TT
 * @returns {{lon:number, lat:number, dist:number, ra:number, dec:number}}
 *   эклиптические долгота/широта эпохи даты (°), расстояние (км), RA/Dec (°)
 */
export function moonPosition(jdTT) {
  const T = centuries(jdTT);
  const T2 = T * T, T3 = T2 * T, T4 = T3 * T;
  const Lp = norm360(218.3164477 + 481267.88123421 * T - 0.0015786 * T2 + T3 / 538841 - T4 / 65194000);
  const D = norm360(297.8501921 + 445267.1114034 * T - 0.0018819 * T2 + T3 / 545868 - T4 / 113065000);
  const M = norm360(357.5291092 + 35999.0502909 * T - 0.0001536 * T2 + T3 / 24490000);
  const Mp = norm360(134.9633964 + 477198.8675055 * T + 0.0087414 * T2 + T3 / 69699 - T4 / 14712000);
  const F = norm360(93.272095 + 483202.0175233 * T - 0.0036539 * T2 - T3 / 3526000 + T4 / 863310000);
  const A1 = norm360(119.75 + 131.849 * T);
  const A2 = norm360(53.09 + 479264.29 * T);
  const A3 = norm360(313.45 + 481266.484 * T);
  const E = 1 - 0.002516 * T - 0.0000074 * T2;
  const E2 = E * E;

  let sl = 0, sr = 0, sb = 0;
  for (let i = 0; i < LR.length; i++) {
    const t = LR[i];
    const arg = (t[0] * D + t[1] * M + t[2] * Mp + t[3] * F) * DEG;
    const m = Math.abs(t[1]);
    const f = m === 1 ? E : m === 2 ? E2 : 1;
    if (t[4]) sl += t[4] * f * Math.sin(arg);
    if (t[5]) sr += t[5] * f * Math.cos(arg);
  }
  for (let i = 0; i < B.length; i++) {
    const t = B[i];
    const arg = (t[0] * D + t[1] * M + t[2] * Mp + t[3] * F) * DEG;
    const m = Math.abs(t[1]);
    const f = m === 1 ? E : m === 2 ? E2 : 1;
    sb += t[4] * f * Math.sin(arg);
  }
  sl += 3958 * sind(A1) + 1962 * sind(Lp - F) + 318 * sind(A2);
  sb += -2235 * sind(Lp) + 382 * sind(A3) + 175 * sind(A1 - F) + 175 * sind(A1 + F) +
    127 * sind(Lp - Mp) - 115 * sind(Lp + Mp);

  const lon = norm360(Lp + sl / 1e6);
  const lat = sb / 1e6;
  const dist = 385000.56 + sr / 1000;
  const eps = obliquity(T);
  const [ra, dec] = eclToEq(lon, lat, eps);
  return { lon, lat, dist, ra, dec };
}

/**
 * Фаза Луны.
 * @returns {{illum:number, phaseAngle:number, elong:number, waxing:boolean, age:number}}
 *   illum — освещённая доля 0..1; elong — элонгация по долготе 0..360 (0 — новолуние,
 *   180 — полнолуние); age — «возраст» в сутках от новолуния (оценка по элонгации).
 */
export function moonPhase(jdTT, moon = moonPosition(jdTT), sun = sunPosition(jdTT)) {
  const psi = acosd(cosd(moon.lat) * cosd(moon.lon - sun.lon));
  const R = sun.dist * AU_KM;
  const i = atan2d(R * sind(psi), moon.dist - R * cosd(psi));
  const illum = (1 + cosd(i)) / 2;
  const elong = norm360(moon.lon - sun.lon);
  return {
    illum,
    phaseAngle: i,
    elong,
    waxing: elong < 180,
    age: (elong / 360) * 29.530588853,
  };
}

/** Экваториальный горизонтальный параллакс Луны, градусы. */
export const moonParallax = (distKm) => Math.asin(EARTH_RADIUS_KM / distKm) / DEG;
